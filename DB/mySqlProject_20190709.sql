...
-- user별 수면 정보 조회
create view user_sleeping_view as(
	select user.user_id, user.name, sleeping.sleeping_date, 
		sleeping.sleeping_seconds, sleeping.snoring_seconds
	from user, sleeping
	where user.user_id=sleeping.user_id
    order by user_id, sleeping_date
);
select * from user_sleeping_view;
...
-- user별 음주 정보 조회
create view user_drinking_view as(
	select user.user_id, user.name, 
		drinking.drinking_date, drinking.alcohol_content
	from user, drinking
	where user.user_id=drinking.user_id
    order by user_id, drinking_date
);
select * from user_drinking_view;
